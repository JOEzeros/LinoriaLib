...
local SaveManager = {} do
	...
	function SaveManager:Save(name)
		...
		for idx, toggle in next, Toggles do
			if self.Ignore[idx] then  end

			table.insert(data.objects, self.Parser[toggle.Type].Save(idx, toggle))
		end

		for idx, option in next, Options do
			if not self.Parser[option.Type] then  end
			if self.Ignore[idx] then  end
	...
	function SaveManager:BuildConfigSection(tab)
		...
		section:AddInput('SaveManager_ConfigName',    { Text = 'New Config Name' })
		...
		section:AddButton('Create', function()
			...
			self.Library:Notify(string.format('Config created %q', name))
			...
			self.Library:Notify(string.format('config loaded %q', name))
		end)
